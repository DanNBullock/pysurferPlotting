...
def singleNiftiHeatmap(niftiPath1, subject_id, subjects_dir):   
        ...
        fib1_surf_data = io.project_volume_data(filepath=niftiPath1, hemi=side,  subject_id=subject_id, projsum='max', smooth_fwhm=0, projmeth='dist',  projarg=steps)
...
#        fib2mean=np.true_divide(fib2_surf_data.sum(),(fib2_surf_data!=0).sum())
        ...
        brain.add_data(fib1_surf_data, 0, 200 ,10 ,colormap="gist_rainbow", alpha=.6,
                       hemi=side)
